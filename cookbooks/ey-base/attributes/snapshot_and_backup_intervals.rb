...
default.set_unless['snapshot_hour'] = if ['24', ''].include?(node['backup_interval'].to_s)
  "9" # 0100 PDT, per support's request, instances run in UTC
else
  "*/#{node['backup_interval']}"
end

default.set_unless['snapshot_minute'] = '1'

default.set_unless['backup_hour'] = node['snapshot_hour']
default.set_unless['backup_minute'] = (node['snapshot_minute'].to_i + 10).to_s

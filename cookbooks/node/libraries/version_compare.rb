...
class Engineyard::Version

  include Comparable

  VERSION_PATTERN = '[0-9]+(?>\.[0-9a-zA-Z]+)*(-[0-9A-Za-z-]+(\.[0-9A-Za-z-]+)*)?' # :nodoc: -- Need to handle -r1 as post
  ...
  def self.new version # :nodoc:
    return super unless Engineyard::Version == self
  ...
  def initialize version
    ...
    @version = version.to_s.strip.gsub("-r",".")  # Convert a.b.c-rd to a.b.c.d
    @segments = nil
  end
  ...
  def pretty_print q # :nodoc:
    q.text "Engineyard::Version.new(#{version.inspect})"
  end
  ...
  def <=> other
    return unless Engineyard::Version === other
...
class Engineyard::Requirement
  ...
  PATTERN_RAW = "\\s*(#{quoted})?\\s*(#{Engineyard::Version::VERSION_PATTERN})\\s*" # :nodoc:
  ...
  DefaultRequirement = [">=", Engineyard::Version.new(0)]
  ...
  ##
  # Factory method to create a Engineyard::Requirement object.  Input may be
  ...
  def self.create input
    case input
    when Engineyard::Requirement then
      input
    when Engineyard::Version, Array then
      new input
    when '!' then
      source_set
    else
  ...
  # be a String or a Engineyard::Version.
  #
  ...
  #     parse("> 1.0")                 # => [">", "1.0"]
  #     parse("1.0")                   # => ["=", "1.0"]
  #     parse(Engineyard::Version.new("1.0")) # => ["=,  "1.0"]

  def self.parse obj
    return ["=", obj] if Engineyard::Version === obj
  ...
  def satisfied_by? version
    raise ArgumentError, "Need a Engineyard::Version: #{version.inspect}" unless
      Engineyard::Version === version
  ...
  def == other # :nodoc:
    Engineyard::Requirement === other and to_s == other.to_s
  end
